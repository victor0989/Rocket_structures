...
def create_down_adapter():
    # Partes sólidas
    base_ext = trimesh.creation.cylinder(radius=(101 / 2 + 2), height=24)
    base_ext.apply_translation([0, 0, 24 / 2])

    cone = trimesh.creation.cone(radius=30 / 2, height=48)
    cone.apply_translation([0, 0, 24 + 48 / 2])

    tail = trimesh.creation.cylinder(radius=30 / 2, height=5)
    tail.apply_translation([0, 0, 24 + 48 + 5 / 2])

    solid = combine_meshes([base_ext, cone, tail])

    # Huecos internos
    hole_base = trimesh.creation.cylinder(radius=101 / 2, height=25)
    hole_base.apply_translation([0, 0, 24 / 2])

    hole_cone = trimesh.creation.cone(radius=(30 / 2) - 2, height=49)
    hole_cone.apply_translation([0, 0, 24 + 48 / 2])

    hole_tail = trimesh.creation.cylinder(radius=(30 / 2) - 2, height=6)
    hole_tail.apply_translation([0, 0, 24 + 48 + 5 / 2])

    hole = combine_meshes([hole_base, hole_cone, hole_tail])

    # Booleana
    final_mesh = solid.difference(hole)

    return final_mesh

def create_space_cup():
    # Un cilindro que simula una cápsula
    cup = trimesh.creation.cylinder(radius=12, height=25)
    cup.apply_translation([0, 0, 25 / 2])
    return cup


def create_kcup_holder():
    components = []

    # Base (posición estimada)
    base = trimesh.creation.box(extents=[160, 10, 4])
    base.apply_translation([0, -76, 2])
    ...
    for i, x in enumerate(positions):
        kcup = create_space_cup()
        kcup.apply_translation([x, -((25.4 * 6) / 8) - 10, inserts[i]])
        components.append(kcup)

    return combine_meshes(components)

def create_spacecup_array():
    cups = []
    for i in range(3):
        cup = create_space_cup()
        cup.apply_translation([i * 60 - 60, 0, 0])
        cups.append(cup)
    return combine_meshes(cups)
...
def main():
    components = [
        ...
        *surface_panels,
        create_down_adapter(),
        create_kcup_holder(),
        create_spacecup_array()

    ]

    model = combine_meshes(components)
    plot_mesh(model, filename="falcon_parker_star_trek_loaded_v1_SCAD.png")
    model.export('falcon_parker_star_trek_loaded_v1_SCAD.stl')
    print("Modelo exportado a falcon_parker_star_trek_loaded_v1_SCAD.stl, versión cargada Trek")
