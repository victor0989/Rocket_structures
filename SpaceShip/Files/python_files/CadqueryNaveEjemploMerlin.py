...
def create_sensors():
    sensors = []
    for pos in [[1.0, 0, FUSELAGE_LENGTH - 2], [-1.0, 0, FUSELAGE_LENGTH - 2]]:
...
# ---- ADAPTACIÓN DE WALKWAY DE CADQUERY A TRIMESH ----
def create_walkway_structure():
    walkway = trimesh.creation.box(extents=[90, 60, 6])
    walkway.apply_translation([0, 0, -4])
    chamfer_edges = trimesh.creation.cylinder(radius=1.5, height=6)
    chamfer_edges.apply_translation([43, 28, -4])
    return [walkway]

# ---- ADAPTACIÓN DE TILES A TRIMESH ----
def create_tile_grid():
    tiles = []
    tile_size = 10
    height_range = [1.0, 2.5]
    for x in range(-30, 40, tile_size):
        for y in range(-30, 40, tile_size):
            h = np.random.uniform(*height_range)
            tile = trimesh.creation.box(extents=[tile_size * 0.9, tile_size * 0.9, h])
            tile.apply_translation([x, y, h / 2 - 6])
            tiles.append(tile)
    return tiles
...
def plot_mesh(mesh):
    ...
    plt.tight_layout()
    plt.savefig("/mnt/c/Users/PC/PycharmProjects/pythonProject1/NaveCompleta_con_EstructuraIndustrial.png", dpi=300)
    plt.show()

# ------------------- MAIN -------------------
def main():
    parts = []
    parts.extend(create_tile_grid())  # Terreno industrial con tiles
    parts.extend(create_walkway_structure())  # Plataforma tipo hangar
